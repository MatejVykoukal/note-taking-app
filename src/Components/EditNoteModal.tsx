...
const EditNoteModal: React.FC<Props> = ({ title:initialTitle, notes}) => {
  const { closeAllModals } = useGlobalModal();
  const { createNewNote } = useNotes();

  const [title, setTitle] = useState({ value: initialTitle, validationError: "" });
  const [note, setNote] = useState({ value: notes, validationError: "" });
